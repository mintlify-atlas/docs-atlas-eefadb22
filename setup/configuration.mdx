...
The MCP server includes optional support for [Matomo](https://matomo.org) web analytics tracking.

<ParamField path="MATOMO_SITE_ID" type="string" default="">
  Matomo site ID for analytics tracking.

  When unset or empty, Matomo tracking is disabled.
...
<ParamField path="MATOMO_AUTH" type="string" default="">
  Matomo authentication token for server-side tracking.

  Required when `MATOMO_SITE_ID` is set. The token authenticates tracking requests to the Matomo API.
